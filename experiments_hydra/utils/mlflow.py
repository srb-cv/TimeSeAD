...
def save_active_run_id(output_dir: str) -> Optional[Path]:
    ...
    output_path = Path(output_dir) / "mlflow_run_id.txt"
    output_path.write_text(active_run.info.run_id, encoding="utf-8")
    return output_path


@contextlib.contextmanager
def start_mlflow_run(cfg, run_name: Optional[str] = None) -> Iterator[MLflowMetricLogger]:
    cfg = to_plain_config(cfg)
    tracking_uri = cfg["experiment"].get("tracking_uri")
    if tracking_uri is None:
        tracking_uri = f"file://{PROJECT_ROOT}/mlruns_hydra"
